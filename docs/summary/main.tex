...
\textbf{设置.} 采用\emph{两块对接}极坐标 O 网格（$\theta\in[0,\pi]$ 与 $[\pi,2\pi]$，各 $121\times61$），周向接缝为块–块内部接口（避免自周期接缝）；内边界壁面无滑移，外边界上游速度入口、下游压力出口；AC（$t_{end}=3\times10^4$、每外层步耦合）。Re$=$200 拟用 dual-time。\par
\textbf{结果.} 两块对接已实现并推进（$Cl\approx0$，对称性正确）；但当前 $C_d\approx2.3$、尾迹无明显回流区，与 Dennis--Chang 基准不符，判为\emph{待校验}——疑与极坐标远场进出口（外圆下游压力出口/上游速度入口）设置有关，需进一步调试。
