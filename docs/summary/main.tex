...
\textbf{结果.} AC 收敛；与复合 Darcy--Brinkman 剖面的 $L^2$ 相对偏差：$\varepsilon=0.3/0.6/0.9$ 分别为 $0.41/0.21/0.21$（粗网格 $81\times21$）。\n加密网格（$161\times81$）跑到约 1/3 步长时偏差反而更大（$1.01/0.79/0.23$），说明 AC 显式逐块耦合在细网格上收敛更慢；\textbf{本轮不再进一步加密，按最新情况记录}，列为\emph{待改进}（可在细网格上增加外迭代/欠松弛改进）。
...
\end{figure}
\caption{方腔 $u(x=0.5)$（vs Ghia）、槽道剖面（vs Poiseuille）、Porous Plug 三孔隙率、LTNE 单例（点：CFD；线：解析/基准）。}
\end{figure}
