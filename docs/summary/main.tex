...
加密网格（$161\times81$，同一 block-Jacobi 格式，$t_{end}=3\times10^5$）运行至 Kstep$=10^5$（约 1/3）时偏差反而更大（$1.01/0.79/0.23$），且逐块 AC 耦合在细网格上收敛明显变慢（外迭代残差停在 $\sim0.1$--$0.18$），该加密运行随后**超时终止、未收敛**。\textbf{本轮不再进一步加密或调试，按最新情况记录}（以粗网格收敛解为准），列为\emph{待改进}（细网格需增加内迭代/欠松弛或界面隐式处理）。
