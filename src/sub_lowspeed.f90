!==============================================================================
! Low-speed (incompressible) flow solver
! SIMPLE algorithm on collocated rectangular grids (non-uniform allowed)
! Variables (primitive, cell-centered):  U(1..5) = [rho, u, v, w, T]
! Pressure is stored in B%p (cell-centered, ghost layers included)
! Steady state, no turbulence model.
!==============================================================================
  module lowspeed_work
   use precision_EC
   implicit none
   ...
   integer :: nxw, nyw, nzw
  end module lowspeed_work

!==============================================================================
! Initialize a low-speed block with primitive variables
!==============================================================================
  subroutine init_lowspeed_block(B)
   use Global_Var
   implicit none
   ...
   do i=1-LAP,B%nx+LAP-1
     ...
     B%p(i,j,k)=LS_P_out
   enddo; enddo; enddo
  end subroutine init_lowspeed_block
...
!==============================================================================
  subroutine lowspeed_boundary_block(nMesh, mBlock)
   use Global_Var
   use const_var
   use lowspeed_work
   implicit none
   ...
   interface
     subroutine lowspeed_ghost(bc, face_s, ig,jg,kg, i2,j2,k2, B, uin_x,uin_y,uin_z)
       use precision_EC
       use Global_Var
     ...
     subroutine lowspeed_face_flux_bc(bc, face_s, i1,j1,k1, B, uin_x,uin_y,uin_z)
       use precision_EC
       use Global_Var
       integer:: bc, face_s, i1,j1,k1
       Type (Block_TYPE),pointer:: B
       real(PRE_EC):: uin_x,uin_y,uin_z
     end subroutine lowspeed_face_flux_bc
      subroutine lowspeed_inlet_velocity(B, uin_x, uin_y, uin_z)
        use precision_EC
        use Global_Var
        Type (Block_TYPE),pointer:: B
        real(PRE_EC):: uin_x,uin_y,uin_z
      end subroutine lowspeed_inlet_velocity
   end interface
...
!  Compute inlet velocity components (velocity inlet / mass-flow inlet / pressure inlet)
   ...
   do ksub=1, B%subface
     ...
     case(1)  ! i- face
       do n=1, LAP
         ...
         do k=kb,ke-1; do j=jb,je-1
           call lowspeed_ghost(Bc%bc, face_s, i1,j,k, i2,j,k, B, uin_x,uin_y,uin_z)
         enddo; enddo
       enddo
       do k=kb,ke-1; do j=jb,je-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=ib, j1=j, k1=k, B=B, &
     ...
     case(4)  ! i+ face
       ...
       enddo
       do k=kb,ke-1; do j=jb,je-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=ie, j1=j, k1=k, B=B, &
                                    uin_x=uin_x, uin_y=uin_y, uin_z=uin_z)
       enddo; enddo
     case(2)  ! j- face
       do n=1, LAP
         ...
         do k=kb,ke-1; do i=ib,ie-1
           call lowspeed_ghost(Bc%bc, face_s, i,j1,k, i,j2,k, B, uin_x,uin_y,uin_z)
         enddo; enddo
       enddo
       do k=kb,ke-1; do i=ib,ie-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=i, j1=jb, k1=k, B=B, &
                                    uin_x=uin_x, uin_y=uin_y, uin_z=uin_z)
       enddo; enddo
     case(5)  ! j+ face
       do n=1, LAP
       ...
       enddo
       do k=kb,ke-1; do i=ib,ie-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=i, j1=je, k1=k, B=B, &
                                    uin_x=uin_x, uin_y=uin_y, uin_z=uin_z)
       enddo; enddo
     case(3)  ! k- face
       do n=1, LAP
         ...
         do j=jb,je-1; do i=ib,ie-1
           call lowspeed_ghost(Bc%bc, face_s, i,j,k1, i,j,k2, B, uin_x,uin_y,uin_z)
         enddo; enddo
       enddo
       do j=jb,je-1; do i=ib,ie-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=i, j1=j, k1=kb, B=B, &
                                    uin_x=uin_x, uin_y=uin_y, uin_z=uin_z)
       enddo; enddo
     case(6)  ! k+ face
       do n=1, LAP
       ...
       enddo
       do j=jb,je-1; do i=ib,ie-1
         call lowspeed_face_flux_bc(Bc%bc, face_s, i1=i, j1=j, k1=ke, B=B, &
                                    uin_x=uin_x, uin_y=uin_y, uin_z=uin_z)
       enddo; enddo
     end select
   enddo
  end subroutine lowspeed_boundary_block
...
!==============================================================================
  subroutine lowspeed_ghost(bc, face_s, ig,jg,kg, i2,j2,k2, B, uin_x,uin_y,uin_z)
   ...
   interface
     subroutine lowspeed_set_face_speed(face_s, ig,jg,kg, i2,j2,k2, B, wall_mode, uw,vw,ww)
       use precision_EC
       use Global_Var
     ...
     end subroutine lowspeed_set_face_speed
   end interface
...
!  Wall tangential velocity (lid-driven: j+ face moves at LS_U_lid in x)
   ...
   case(BC_Wall)   ! no-slip wall (possibly lid-driven)
     call lowspeed_set_face_speed(face_s, ig,jg,kg, i2,j2,k2, B, &
          .true., uw, vw, ww)
...
!==============================================================================
  subroutine lowspeed_store_face_flux(face_s, i1,j1,k1, flux)
   use lowspeed_work
   implicit none
  ...
  end subroutine lowspeed_store_face_flux

!==============================================================================
! Compute face mass fluxes.
! Interior faces: Rhie-Chow momentum interpolation.
! Boundary faces: already set by lowspeed_boundary_block.
!==============================================================================
  subroutine lowspeed_face_flux(nMesh, mBlock)
   use Global_Var
   use const_var
   use lowspeed_work
   implicit none
...
!==============================================================================
  subroutine conv_face_delta(ph_up, ph_up2, ph_dn, delta)
   use Global_Var
   implicit none
  ...
  end subroutine conv_face_delta
...
!==============================================================================
  subroutine lowspeed_conv_high(nMesh, mBlock, ivar)
   use Global_Var
   use const_var
   use lowspeed_work
   implicit none
...
!    e face
     Fe = cf*Fi(i+1,j,k)
     if(Fe > 0.d0) then
       call conv_face_delta(phP, phW, phE, delta)
       conv = conv + Fe*delta
     else
       call conv_face_delta(phE, phEE, phP, delta)
       conv = conv + Fe*delta
     endif
!    w face (outward normal is -x: accumulate with outward flux -Fw)
     Fw = cf*Fi(i,j,k)
     if(Fw > 0.d0) then
       call conv_face_delta(phW, phWW, phP, delta)
       conv = conv - Fw*delta
     else
       call conv_face_delta(phP, phE, phW, delta)
       conv = conv - Fw*delta
     endif
!    n face
     Fn = cf*Fj(i,j+1,k)
     if(Fn > 0.d0) then
       call conv_face_delta(phP, phS, phN, delta)
       conv = conv + Fn*delta
     else
       call conv_face_delta(phN, phNN, phP, delta)
       conv = conv + Fn*delta
     endif
!    s face (outward normal is -y: accumulate with outward flux -Fs)
     Fs = cf*Fj(i,j,k)
     if(Fs > 0.d0) then
       call conv_face_delta(phS, phSS, phP, delta)
       conv = conv - Fs*delta
     else
       call conv_face_delta(phP, phN, phS, delta)
       conv = conv - Fs*delta
     endif
!    t face
     Ft = cf*Fk(i,j,k+1)
     if(Ft > 0.d0) then
       call conv_face_delta(phP, phB, phT, delta)
       conv = conv + Ft*delta
     else
       call conv_face_delta(phT, phTT, phP, delta)
       conv = conv + Ft*delta
     endif
!    b face (outward normal is -z: accumulate with outward flux -Fb)
     Fb = cf*Fk(i,j,k)
     if(Fb > 0.d0) then
       call conv_face_delta(phB, phBB, phP, delta)
       conv = conv - Fb*delta
     else
       call conv_face_delta(phP, phT, phB, delta)
       conv = conv - Fb*delta
     endif
     conv_src(i,j,k) = conv
   enddo; enddo; enddo
  end subroutine lowspeed_conv_high
...
!==============================================================================
  subroutine lowspeed_momentum(nMesh, mBlock, dir)
   use Global_Var
   use const_var
   use lowspeed_work
   implicit none
...
!  high-order convection correction (deferred), computed once per sweep
   if(LS_Scheme > 1) call lowspeed_conv_high(nMesh, mBlock, dir+1)
   do it=1, INNER
     do k=1,nz-1
     do j=1,ny-1
     do i=1,nx-1
...
!==============================================================================
  subroutine lowspeed_energy(nMesh, mBlock)
   ...
   Type (Block_TYPE),pointer:: B
   integer:: i,j,k, nx,ny,nz, iter
    real(PRE_EC):: rho, kcond, cph, alpha, ap, aE,aW,aN,aS,aT,aB, src, apstab
    real(PRE_EC):: De,Dwe,Dn,Ds,Dt,Db, Fe,Fw,Fn,Fs,Ft,Fb, unew, uold
   integer,parameter:: INNER=3
...
!  high-order convection correction (deferred)
   if(LS_Scheme > 1) call lowspeed_conv_high(nMesh, mBlock, 5)
   do iter=1, INNER
     do k=1,nz-1
     do j=1,ny-1
     do i=1,nx-1
...
!  (re)allocate work arrays if size changed
   ...
   res0 = 0.d0
   do iter=1, LS_Max_Iter
!    boundary conditions (ghost cells + boundary face fluxes)
     call lowspeed_boundary_block(nMesh, mBlock)

!    momentum prediction
     call lowspeed_momentum(nMesh, mBlock, 1)
     call lowspeed_momentum(nMesh, mBlock, 2)
     call lowspeed_momentum(nMesh, mBlock, 3)

!    face fluxes (interior Rhie-Chow; boundary set by BC)
     call lowspeed_face_flux(nMesh, mBlock)

!    pressure correction + velocity/pressure update
     call lowspeed_pressure_correction(nMesh, mBlock, res_p)
...
!    (initial u=0,p=0 gives an identically-zero mass residual on sweep 1)
   ...
   if(my_id == 0) then
     print*, "Low-speed solver Block", B%Block_no, " SIMPLE iterations:", iter, &
             " res_p:", res_p
   endif
