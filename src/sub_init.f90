...
!  Mesh file: PLOT3D format;   
   ...
   allocate(MP%Block(Num_block))               ! ??????????ïB
   call set_size_blocks                        ! ?Ú…?????§³
   call allocate_mem_Blocks(1)                 ! ?????????????????

!  ?Ú…???????
   allocate(MP%Res_max(NVAR),MP%Res_rms(NVAR))   ! ???§Ó?????????§Ó? 
   MP%Kstep=0
   MP%tt=0.d0
   Num_Cell=0   ! ???????
    do m=1,Num_Block
...
! ??????????????????????????m2 
  subroutine Creat_Mesh(m1,m2)
   ...
   allocate(MP2%Block(NB))   ! ??MP2?§Õ?????????????ïB
     Num_Cell=0
   do m=1,NB
     B1=>MP1%Block(m)
     B2=>MP2%Block(m)
...
!    ???????????
     ...
     do ksub=1, Bsub
	   ...
	   Bc2%ib=(Bc1%ib-1)/2+1   ! ?????????¡À???????
	   Bc2%ie=(Bc1%ie-1)/2+1   ! ?????????¡À???????
	   Bc2%jb=(Bc1%jb-1)/2+1
	   Bc2%je=(Bc1%je-1)/2+1
	   Bc2%kb=(Bc1%kb-1)/2+1
	   Bc2%ke=(Bc1%ke-1)/2+1
 	   
	   Bc2%ib1=(Bc1%ib1-1)/2+1   ! ?????????¡À???????
	   Bc2%ie1=(Bc1%ie1-1)/2+1   ! ?????????¡À???????
...
! see:       http://turbmodels.larc.nasa.gov/spalart.html
   ...
   call Boundary_condition_onemesh(Num_Mesh)     ! ??????? ???Ú…Ghost Cell?????
...
!     call output (nMesh)
     if(nMesh .gt. 1) then
	   ...
	   call Boundary_condition_onemesh(nMesh-1)         ! ??????? ???Ú…Ghost Cell?????
...
!  ???????? LAP????????  ???????????????????????          
     ...
     allocate( B%Res(NVAR1,-1:nx+1,-1:ny+1,-1:nz+1))        !  ?§Ó?
...
! --------?????????????????????1??????????----------------------------
 ...
 !----???????? (cell-centered, LAP ghost layers, ???§áúY????)
