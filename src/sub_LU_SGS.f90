!  Compute DU=U(n+1)-U(n) using the LU-SGS method
!  Code by Li Xinliang, 2011-12-29
!-----------------------------------------------------------------------------------------
    subroutine  du_LU_SGS(nMesh,mBlock,Sfac1)                          ! Compute DU=U(n+1)-U(n) using the LU-SGS method
    use Global_Var
    use Flow_Var 
    implicit none
    ...
    real(PRE_EC):: Sfac1    ! Used for dual time-stepping

     NV=Mesh(nMesh)%NVAR
	 B => Mesh(nMesh)%Block(mBlock)                 ! Block mBlock of mesh level nMesh
     nx=B%nx; ny=B%ny; nz=B%nz

! LU-SGS of two sweeps
!----------------------------------
!   Forward sweep from i=1,j=1,k=1 to i=nx-1,j=ny-1,k=nz-1
!   Sweep over planes defined by i+j+k=plane
!   w_LU is the relaxation factor (between 1 and 2); increasing w_LU improves stability but reduces convergence rate
...
!$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE) SHARED(plane,nx,ny,nz,NV,B,Lci,Lcj,Lck,Lvi,Lvj,Lvk,gamma,w_LU,If_viscous)
	 ...
	 do j=1,ny-1
	 i=plane-k-j
	 if( i .lt. 1 .or. i .gt. nx-1) cycle    ! Outside this plane
       alfa(1:NV)=B%vol(i,j,k)/B%dt(i,j,k)+w_LU*(Lci(i,j,k)+Lcj(i,j,k)+Lck(i,j,k))     ! Diagonal term
     if(If_viscous .eq. 1)  then
	   alfa(1:NV)=alfa(1:NV)+2.d0*(Lvi(i,j,k)+Lvj(i,j,k)+Lvk(i,j,k))          
       if(NV .eq. 6) then
	     alfa(6)=alfa(6)+(Lvi(i,j,k)+Lvj(i,j,k)+Lvk(i,j,k))      ! Further increase diagonal (considering sigma_SA=2.d0/3.d0)
!	   else if(NV .eq. 7) then
!	      alfa(6)=alfa(6)+0.09*B%U(6,i,j,k)/B%U(1,i,j,k)*Re*B%vol(i,j,k)                          ! Handle source term stiffness
!         alfa(7)=alfa(7)+2.d0*0.0828*B%U(6,i,j,k)/B%U(1,i,j,k)*Re*B%vol(i,j,k)
	   endif
     endif
	 	 alfa(1:NV)=alfa(1:NV)+Sfac1*B%vol(i,j,k)         ! Single time-stepping: Sfac1=0
		  		   
	 if(i.ne. 1) then
!                                              Flux difference, used to approximate A*W (See Blazek's book, page 208)
	   ...
	   dui(1:NV)=0.d0                             ! No point on the left
      endif
