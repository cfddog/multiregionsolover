...
!$ if(my_id ==0) then         ! ??????????§Ö???? ??openmp???????????????
!$OMP Parallel
!$  print*, "omp run ..."
!$OMP END parallel
!$ endif 

   allocate( Mesh(Num_Mesh) )                                 ! ????????? ?????? ?????????????ïB??
   
   if(my_id .eq. 0)  call check_mesh_multigrid               ! ??????????????????????????,???Ú…?????????????
   
   call Init                               ! ?????????????????—¨???????
   call set_control_para                   ! ?Ú…????????????????????????????????????????????????????????
...
!   ?????????????????????§á????????????????????ýD????????
       ...
       case(Time_RK3)                                  ! 3??RK: ???3?????(stage)
         ...
         do KRK=1,3
           ...
           call Boundary_condition_onemesh(1)          ! ??????? (?Ú…Ghost Cell???)
       ...
       case(Time_Dual_LU_SGS)                          ! ????????: ?????
         do kt_in=1, step_inner_Limit
           ...
           call update_buffer_onemesh(1)               ! ?????????????
           call update_Ts_buffer_onemesh(1)            ! ???????????????
           call comput_max_Res_onemesh(1)              ! ???????/???????§Ó?
           max_res=Mesh(1)%Res_rms(1)
           do m=1,Mesh(1)%NVAR
             max_res=max(max_res,Mesh(1)%Res_rms(m))
           enddo
           if( max_res .le. Res_Inner_Limit) exit      ! ???§Ó??????????????
         enddo
         if(my_id .eq. 0) then
